    ...
    public function load(ObjectManager $manager)
    {
       ...
       $categories = Array();
        ...
        foreach (range(1, 8) as $i) {
            ...
            $product->setTitle($this->getRandomPostTitle())
                    ->setDateCreate(new \DateTime('now - '.$i.'days'))
                    ->setCategory( $categories[ rand(0,2) ]  )
                    ->setPrice( (float) rand(1000, 3000) / 10 )
                    ->setDescription( $this->getPostContent() )
                    ->setOrigin('France')
                    ->setImage($image)
                    ;
